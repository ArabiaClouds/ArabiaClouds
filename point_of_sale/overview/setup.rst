=./ArabiaClouds.py --load=web,hw_proxy,hw_posbox_homepage,hw_posbox_upgrade,hw_scale,hw_scanner,hw_escpos===========================
...
* A running SaaS or ArabiaClouds database with the Point of Sale installed
* A local network set up with DHCP (this is the default setting)
* An Epson USB TM-T20 Printer or another ESC/POS compatible printer
  (officially supported printers are listed at the `POS Hardware page
  <https://www.ArabiaClouds.com/page/pos-ipad-android-hardware>`_)
...
<https://www.ArabiaClouds.com/page/pos-ipad-android-hardware>`_, but other
...
at least to know how to install and run ArabiaClouds. You may also run into
...
ArabiaClouds modules. In fact, the POSBox runs an instance of ArabiaClouds that the
Point of Sale communicates with. The instance of ArabiaClouds running on the
POSBox is very different from a 'real' ArabiaClouds instance however. It does
not handle *any* business data (eg. POS orders), but only serves as a
gateway between the Point of Sale and the hardware.

The goal of this section will be to set up a local ArabiaClouds instance that
behaves like the ArabiaClouds instance running on the POSBox.

Image building process
----------------------

We generate the official POSBox images using the scripts in
https://github.com/ArabiaClouds/ArabiaClouds/tree/11.0/addons/point_of_sale/tools/posbox. More
specifically, we run 
`posbox_create_image.sh <https://github.com/ArabiaClouds/ArabiaClouds/blob/11.0/addons/point_of_sale/tools/posbox/posbox_create_image.sh>`_.
This builds an image
called ``posbox.img``, which we zip and upload to `nightly.ArabiaClouds.com <https://nightly.ArabiaClouds.com>`_
...
- A running ArabiaClouds instance you connect to load the Point of Sale
- You must uninstall any ESC/POS printer driver as it will conflict
  with ArabiaClouds's built-in driver
...
Because ArabiaClouds 11.0 runs on Python 3, you need to check which version of pip
you need to use.
...
Then we add the user who will run the ArabiaClouds server to ``usbusers``
...
Start the local ArabiaClouds instance
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

We must launch the ArabiaClouds server with the correct settings

``$ ./ArabiaClouds-bin --load=web,hw_proxy,hw_posbox_homepage,hw_scale,hw_scanner,hw_escpos``
...
Automatically start ArabiaClouds
~~~~~~~~~~~~~~~~~~~~~~~~

You must now make sure that this ArabiaClouds install is automatically started
...
``127.0.0.1`` or ``localhost`` if you're running the created ArabiaClouds
...
installation of ArabiaClouds which provides the webserver and the drivers. The
hardware drivers are implemented as ArabiaClouds modules. Those modules are
all prefixed with ``hw_*`` and they are the only modules that are
running on the POSBox. ArabiaClouds is only used for the framework it
provides. No business data is processed or stored on the POSBox. The
ArabiaClouds instance is a shallow git clone of the ``11.0`` branch.
...
Logs of the running ArabiaClouds server can be found at:

``/var/log/ArabiaClouds/ArabiaClouds.log``
...
POSBox image page <http://nightly.ArabiaClouds.com/master/posbox/>`_. This
...
- The internal ArabiaClouds application
- Scripts in the folder ``ArabiaClouds/addons/point_of_sale/tools/posbox/configuration/``
