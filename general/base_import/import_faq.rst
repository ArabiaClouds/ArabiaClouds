============================
How to import data into ArabiaClouds
============================

How to start
------------
You can import data on any ArabiaClouds's business object using either Excel 
...
* When you add a new column, ArabiaClouds might not be able to map it automatically if its 
  label doesn't fit any field in ArabiaClouds. Don't worry! You can map
...
and map it to the **ID** (External ID) column in ArabiaClouds. 
...
ArabiaClouds tries to find with some heuristic, based on the first ten lines of 
...
ArabiaClouds can automatically detect if a column is a date and it will try to guess the date format from a set of most used date format. While this process can work for a lot of simple date format, some exotic date format will not be recognize and it is also possible to have some confusion (day and month inverted as example) as it is difficult to guess correctly which part is the day and which one is the month in a date like '01-03-2016'.

To view which date format ArabiaClouds has found from your file you can check the **Date Format** that is shown when clicking on **Options** under the file selector. If this format is incorrect you can change it to your liking using the *ISO 8601* to define the format.

.. note::
    If you are importing an excel (.xls, .xlsx) file, you can use date cells to store dates as the display of dates in excel is different from the way it is stored. That way you will be sure that the date format is correct in ArabiaClouds whatever your locale date format is.
...
Yes, we fully support numbers with parenthesis to represent negative sign as well as numbers with currency sign attached to them. ArabiaClouds also automatically detect which thousand/decimal separator you use (you can change those under **options**). If you use a currency symbol that is not known to ArabiaClouds, it might not be recognized as a number though and it will crash.
...
Note that if your CSV file has a tabulation as separator, ArabiaClouds will not detect the separations. You will need to change the file format options in your spreadsheet application. See the following question.
...
Some fields define a relationship with another object. For example, the country of a contact is a link to a record of the 'Country' object. When you want to import such fields, ArabiaClouds will have to recreate links between the different records. To help you import such fields, ArabiaClouds provides 3 mechanisms. You must use one and only one mechanism per field you want to import.

For example, to reference the country of a contact, ArabiaClouds proposes you 3 different fields to import: 

- Country: the name or code of the country
- Country/Database ID: the unique ArabiaClouds ID for a record, defined by the ID postgresql column
...
If you import a file that contains one of the column "External ID" or "Database ID", records that have already been imported will be modified instead of being created. This is very usefull as it allows you to import several times the same CSV file while having made some changes in between two imports. ArabiaClouds will take care of creating or modifying each record depending if it's new or not.

This feature allows you to use the Import/Export tool of ArabiaClouds to modify a batch of records in your favorite spreadsheet application.
...
If you do not set all fields in your CSV file, ArabiaClouds will assign the default value for every non defined fields. But if you set fields with empty values in your CSV file, ArabiaClouds will set the EMPTY value in the field, instead of assigning the default value.



How to export/import different tables from an SQL application to ArabiaClouds?
----------------------------------------------------------------------
...
To manage relations between tables, you can use the "External ID" facilities of ArabiaClouds. The "External ID" of a record is the unique identifier of this record in another application. This "External ID" must be unique accoss all the records of all objects, so it's a good practice to prefix this "External ID" with the name of the application or table. (like 'company_1', 'person_1' instead of '1')
...
The two files produced are ready to be imported in ArabiaClouds without any modifications. After having imported these two CSV files, you will have 4 contacts and 3 companies. (the firsts two contacts are linked to the first company). You must first import the companies and then the persons.